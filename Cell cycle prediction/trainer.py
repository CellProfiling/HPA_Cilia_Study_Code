...
class Estimator:
    ...
    def resume(self, checkpoint_name):
        try:
            checkpoint = torch.load(
                os.path.join(self.save_path, checkpoint_name),
                map_location=torch.device("cpu"),
            )
        except:
            print(
                "WARNING: failed to resume from checkpoint: "
                + os.path.join(self.save_path, checkpoint_name)
            )
            return False
        ...
        if self.input_channel_num_changed or self.final_changed:
            skip_layers = (
                self.model.first_layer_params_names
                if self.input_channel_num_changed
                else self.model.last_layer_params_names
            )
            print(
                "skipping: ",
                [k for k in pretrained_dict.keys() if any(s in k for s in skip_layers)],
            )
            pretrained_dict = {
                k: v
        ...
        print(
            "resumed from checkpoint {} on epoch: {}".format(
                os.path.join(self.save_path, checkpoint_name), self.start_epoch
            )
        )
        return True
    ...
    def calculate_loss_multichannel(self, output, target, meter, training, iter_size):
        ...
        loss = (
            self.config.loss["bce"] * bce + self.config.loss["dice"] * (1 - dice)
        ) / iter_size
    ...
    def calculate_ssim_l1_loss(self, output, target, meter, training, iter_size):
        ...
        loss = (1 - ssim(output, target)) * self.config.loss["alpha"] + nn.L1Loss()(
            output, target
        ) * (1 - self.config.loss["alpha"])
        if training:
            loss.backward()
    ...
    def calculate_mse_loss(self, output, target, meter, training, iter_size):
        if target.size(1) == 1 and output.size(1) != 1:
            target = target.repeat(1, output.size(1), 1, 1)
        if self.config.architecture == "VQVAE":
            recon_loss = nn.MSELoss()(output[0], target)                
            latent_loss = output[1].mean()
    ...
    def calculate_mse_bce_loss(self, output, target, meter, training, iter_size):
        if target.size(1) == 1 and output.size(1) != 1:
            target = target.repeat(1, output.size(1), 1, 1)
        ce = nn.BCEWithLogitsLoss()(output[:,2,:,:],target[:,2,:,:])
        output_ch2 = torch.sigmoid(output[:,2,:,:])
        #ce = F.binary_cross_entropy_with_logits(output_ch2, target[:,2,:,:])
        dice = dice_loss(output_ch2, target[:,2,:,:])
        #dice = dice_round(output[:,2,:,:], target[:,2,:,:])
        loss_segmentation = self.config.loss["bce"] * ce + (self.config.loss["dice"] * (1 - dice)) / iter_size

        #loss_segmentation = F.binary_cross_entropy_with_logits(output_ch2, target[:,2,:,:])
        #loss_data = nn.MSELoss()(output[:,0:2,:,:]*output[:,2,:,:], target[:,0:2,:,:]*target[:,2,:,:])
        #loss_data = nn.MSELoss()(output[:,0:2,:,:], target[:,0:2,:,:]*target[:,2,:,:])
        loss_data = nn.MSELoss()(output[:,0:2,:,:], target[:,0:2,:,:])
        # Check loss_segmentation range!!! Try nuclei mask and cell mask
        # Start with segmentation nuclei/whole cell as pretrained
        print(loss_data*0.95, loss_segmentation*0.05)
        # Protein differentially expressed based on cell shape map
        loss = 0.95*loss_data + 0.05*loss_segmentation
        if training:
            loss.backward()
        meter["loss"] += loss.data.cpu().numpy()
        return meter
    ...
    def calculate_loss_softmax(self, output, target, meter, training, iter_size):
        ...
        loss = (
            self.config.loss["ce"] * ce
            + self.config.loss["dice_body"] * (1 - dice_body)
            + self.config.loss["dice_border"] * (1 - dice_border)
        ) / iter_size
    ...
    def calculate_loss_sigmoid(self, output, target, meter, training, iter_size):
        ce_body = F.binary_cross_entropy_with_logits(
            output[:, 0, ...], target[:, 2, ...]
        )
        ce_border = F.binary_cross_entropy_with_logits(
            output[:, 1, ...], target[:, 1, ...]
        )
    ...
    def calculate_loss_3ch(self, output, target, meter, training, iter_size):
        ce_body = (
            F.binary_cross_entropy_with_logits(output[:, 2, ...], target[:, 2, ...])
            + F.binary_cross_entropy_with_logits(output[:, 0, ...], target[:, 0, ...])
        ) / 2
        ce_border = F.binary_cross_entropy_with_logits(
            output[:, 1, ...], target[:, 1, ...]
        )
        ...
        output = F.sigmoid(output)
        dice_body = (
            dice_loss(output[:, 0, ...], target[:, 0, ...])
            + dice_loss(output[:, 2, ...], target[:, 2, ...])
        ) / 2
        dice_border = dice_loss(output[:, 1, ...], target[:, 1, ...])
        dice_r_body = (
            dice_round(output[:, 0, ...], target[:, 0, ...])
            + dice_round(output[:, 2, ...], target[:, 2, ...])
        ) / 2
        dice_r_border = dice_round(output[:, 1, ...], target[:, 1, ...])
    ...
    def calculate_mse_kld_loss(self, output, target, mu, logvar): #meter, training, iter_size
        """
        ...
        """
        BCE = nn.MSELoss(output, target)  # mse loss
        ...
        for input, target in zip(inputs, targets):
            ...
            if not training:
                with torch.no_grad():
                    ...
                    #print("Output from VQVAE: ", len(output), output[0].shape, output[1].shape)
                    #if self.config.architecture == "VQVAE": #if len(output) == 2:
                    #    output = output[0] # VQVAE returns img, diff
                    if self.config.loss["type"] == "mse":
                        meter = self.calculate_mse_loss(
                            output, target, meter, training, iter_size
                        )
                    elif self.config.loss["type"] == "mse+bce":
                        meter = self.calculate_mse_bce_loss(
                            output, target, meter, training, iter_size
                        )
                    elif self.config.loss["type"] == "ssim+l1":
                        meter = self.calculate_ssim_l1_loss(
                            output, target, meter, training, iter_size
                        )
                    elif self.config.loss["type"] == "bce":
                        if self.config.activation == "sigmoid":
                            meter = self.calculate_loss_3ch(
                                output, target, meter, training, iter_size
                            )
                        else:
                            meter = self.calculate_loss_softmax(
                                output, target, meter, training, iter_size
                            )
                    elif self.config.loss["type"] == "mse+kld":
                        meter = self.calculate_mse_kld_loss(
                            output, target, meter, training, iter_size
                        )
                    else:
                ...
                if self.config.loss["type"] == "mse":
                    ...
                    if self.config.target_channel_num == 1:
                        meter = self.calculate_loss_single_channel(
                            output, target, meter, training, iter_size
                        )
...
class PytorchTrain:
    ...
    def _run_one_epoch(self, epoch, loader, training=True):
        ...
        pbar = tqdm.tqdm(
            enumerate(loader),
            total=len(loader),
            desc="Epoch {}{}".format(epoch, " eval" if not training else ""),
            ncols=0,
        )
        for i, data in pbar:
            ...
            pbar.set_postfix(
                **{k: "{:.5f}".format(v / (i + 1)) for k, v in avg_meter.items()}
            )
    ...
    def _make_step(self, data, training):
        images = data["image"]
        ytrues = data["mask"]
        print(images.shape, ytrues.shape)
    ...
    def fit(self, train_loader, val_loader, nb_epoch):
        ...
        for epoch in range(self.estimator.start_epoch, nb_epoch):
            ...
            if (
                self.estimator.lr_scheduler is not None
                and epoch >= self.estimator.config.warmup
            ):
    ...
    if scale_tta:
        for scale in [0.8, 1.25]:
            ...
            data = cv2.copyMakeBorder(
                data, 0, (32 - rows % 32), 0, (32 - cols % 32), cv2.BORDER_REFLECT
            )
