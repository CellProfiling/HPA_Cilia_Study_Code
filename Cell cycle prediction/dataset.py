...
class AbstractImageType:
    def __init__(
        self, paths, fn, fn_mapping, preprocessing, has_alpha=False, scale_factor=1.0
    ):
        self.paths = paths
        self.fn = fn
    ...
    def pad_image(self, image, rows, cols):
        ...
        if image.shape[:2] != (rows, cols):
            empty_x = (
                np.zeros((rows, cols, channels), dtype=image.dtype)
                if channels
                else np.zeros((rows, cols), dtype=image.dtype)
            )
...
class RawImageType(AbstractImageType):
    def __init__(
        self, paths, fn, fn_mapping, preprocessing, has_alpha, scale_factor=1.0
    ):
        super().__init__(
            paths, fn, fn_mapping, preprocessing, has_alpha, scale_factor=scale_factor
        )
    ...
    def load_image_files(self, img_type):
        ...
        assert (
            len(rgb_img) == 1 or len(rgb_img) == 3 or len(rgb_img) == 4
        ), f"Invalid image channel number: {len(rgb_img)}"
        ref_channel = None
        ...
        if img_type == 'masks' and im[:,:,2].max() == 1:
            im[:,:,2] = im[:,:,2]*255
        return im
    ...
    def read_label(self):
        ...
        if self.scale_factor != 1.0:
            label = cv2.resize(
                label, (0, 0), fx=self.scale_factor, fy=self.scale_factor
            )
        if label.ndim == 2:
...
class PaddedImageType(BorderImageType):
    def finalyze(self, data):
        ...
        return cv2.copyMakeBorder(
            data, 32, (32 - rows % 32), 32, (32 - cols % 32), cv2.BORDER_REFLECT
        )
...
class AbstractImageProvider:
    def __init__(
        ...
        preprocessing: Dict[AnyStr, Callable],
        has_alpha=False,
        scale_factor=1.0,
    ):
...
class ReadingImageProvider(AbstractImageProvider):
            ...
            if not d.startswith("."):
                ...
                for fn in self.fn_mapping["images"](d):
                    if not os.path.exists(
                        os.path.join(paths["images"], fn)
                    ) and not fn.endswith("None"):
                        skip = True
    ...
    def get_indexes_by_names(self, names):
        indexes = {
            os.path.splitext(name)[0]: idx for idx, name in enumerate(self.im_names)
        }
...
class ImageCropper:
    def __init__(self, img_rows, img_cols, target_rows, target_cols, pad):
        ...
        assert (
            target_rows <= img_rows and target_cols <= img_cols
        ), f"Target size ({target_cols}, {target_rows}) must be smaller than the image size ({img_cols}, {img_rows})"
    ...
    def crop_image(self, image, x, y):
        self.use_crop = ((image.shape[0]) != self.target_rows) or (
            (image.shape[1]) != self.target_cols
        )
        return (
            image[y : y + self.target_rows, x : x + self.target_cols, ...]
            if self.use_crop
            else image
        )
    ...
    def sequential_starts(self, axis=0):
        ...
        if big_segment == small_segment:
            return [0]
        steps = np.ceil(
            (big_segment - self.pad) / (small_segment - self.pad)
        )  # how many small segments in big segment
        if steps == 1:
            return [0]
        new_pad = int(
            np.floor((small_segment * steps - big_segment) / (steps - 1))
        )  # recalculate pad
        starts = [
            i for i in range(0, big_segment - small_segment, small_segment - new_pad)
        ]
...
class Dataset:
    def __init__(
        ...
        self.image_indexes = (
            image_indexes if isinstance(image_indexes, list) else image_indexes.tolist()
        )
        ...
        self.transforms = Compose(
            [
                transforms,
                ToTensor(config.target_channel_num, config.activation) #, normalize),
            ]
        )
        self.croppers = {}
    ...
    def get_cropper(self, image_id, val=False):
        ...
        if image_id not in self.croppers:
            ...
            cropper = ImageCropper(rows, cols, target_rows, target_cols, self.pad)
...
class SequentialDataset(Dataset):
    def __init__(
        self, image_provider, image_indexes, config, stage="test", transforms=None
    ):
        super(SequentialDataset, self).__init__(
            image_provider, image_indexes, config, stage, transforms=transforms
        )
        self.good_tiles = []
...
class TrainDataset(Dataset):
    ...
    def __len__(self):
        return len(self.image_indexes) * max(
...
class ValDataset(SequentialDataset):
    def __init__(
        self, image_provider, image_indexes, config, stage="train", transforms=None
    ):
