...
from dataset import (
    BorderImageType,
    SigmoidBorderImageType,
    PaddedImageType,
    PaddedSigmoidImageType,
    CachingImageProvider,
    InFolderImageProvider,
    TrainDataset,
    ValDataset,
    SequentialDataset,
    set_output_dir,
...
from augmentations.transforms import aug_victor, aug_binarize, aug_minimal
from pytorch_zoo import unet, vqvae, autoencoder
import torch
...
def wsh_nuclei_seeded(config, nuclei_output_name, cell_output_name):
    ...
    """
    masks_files = [
        os.path.splitext(nuclei_output_name[0])[0] + "_output.png",
        os.path.splitext(cell_output_name[0])[0] + "_output.png",
    ]
    target_folder = config.outputs_dir
    sample_folders = list(
        map(
            lambda item: os.path.join(target_folder, item)
    ...
    for sf in sample_folders:
        ...
        nuclei_label = wsh(
            nuclei_img[..., 2] / 255.0,
            0.4,
            1 - (nuclei_img[..., 1] + cell_img[..., 1]) / 255.0 > 0.05,
            nuclei_img[..., 2] / 255,
        ...
        # till here
        nuclei_label_overlay = label2rgb(
            nuclei_label, bg_label=0, bg_color=(0.8, 0.8, 0.8), colors=jet_colors
        )
        ...
        sk = skeletonize(
            ndi.morphology.binary_dilation(
                cell_img[..., 1] / 255.0 > 0.05, iterations=5
            )
        )
        ...
        sk = np.subtract(
            np.asarray(cell_img[..., 2] / 255 > 0.2, dtype=np.int8),
            np.asarray(sk, dtype=np.int8),
        )
        ...
        image_label_overlay = label2rgb(
            cell_label, bg_label=0, bg_color=(0.8, 0.8, 0.8), colors=jet_colors
        )

        cv2.imwrite(
            os.path.join(sf, os.path.splitext(nuclei_output_name[0])[0] + "_label.png"),
            nuclei_label.astype("uint16"),
        )
        print(sk.shape, nuclei_label.shape)
        tmp = np.where(nuclei_label > 0.5, 1, 0)
        cv2.imwrite(
            os.path.join(sf, os.path.splitext(nuclei_output_name[0])[0] + "_binarized.png"),
            tmp.astype("uint8"),
        )

        cv2.imwrite(
            os.path.join(
                sf, os.path.splitext(nuclei_output_name[0])[0] + "_output_color.png"
            ),
            bytescale(nuclei_label_overlay).astype("uint8"),
        )
        cv2.imwrite(
            os.path.join(
                sf, cell_output_name[0].split("_")[0] + "_combined_labels.png"
            ),
            cell_label.astype("uint16"),
        )
        cv2.imwrite(
            os.path.join(
                sf, cell_output_name[0].split("_")[0] + "_combined_labels_color.png"
            ),
            bytescale(image_label_overlay).astype("uint8"),
        )
        cv2.imwrite(
            os.path.join(
                sf, os.path.splitext(cell_output_name[0])[0] + "_skeleton.png"
            ),
            bytescale(sk * 255.0).astype("uint8"),
        )


def download_example_nuclei_dataset(dataset_path):
    os.makedirs(os.path.abspath(dataset_path), exist_ok=True)
    example_nuclei_dataset_url = (
        "https://dl.dropbox.com/s/u5ccevdxewpxdup/HeLa_DAPI_v2.zip"
    )
...
def download_example_hpa_dataset(dataset_path):
    os.makedirs(os.path.abspath(dataset_path), exist_ok=True)
    example_hpa_dataset_url = (
        "https://kth.box.com/shared/static/7pvos5r9wit6k81ai9wakzm492vf4is5.zip"
    )
    example_hpa_dataset = os.path.join(dataset_path, "hpa_dataset.zip")
    download_with_url(example_hpa_dataset_url, example_hpa_dataset, unzip=True)
...
def generate_segmentation_config(
    dataset_path,
    input_channel_files,
    target_channel_files,
...
def start_training(config):
    if config.load_model_from is not None and os.path.exists(config.load_model_from):
        try:
            model = torch.load(config.load_model_from, map_location=torch.device(config.device))
            print("loaded model from " + config.load_model_from)
        except:
            print(
                "Model weight file does not exist or is not valid => Train from scratch"
            )
            model = None
    else:
        model = None
    ...
    fn_mapping = {
        "masks": lambda name: [
            "{}/{}".format(name, fn) for fn in config.target_channel_files
        ],
        "images": lambda name: [
            "{}/{}".format(name, fn) for fn in config.input_channel_files
        ],
        "labels": lambda name: [
            "{}/{}".format(name, fn) for fn in config.input_channel_files
        ],
    ...
    im_type = (
        BorderImageType
        if not config.activation == "sigmoid"
        else SigmoidBorderImageType
    )
    im_val_type = (
        PaddedImageType
        if not config.activation == "sigmoid"
        else PaddedSigmoidImageType
    )
    ...
    paths_valid = {
        "masks": os.path.join(config.dataset_path, "valid"),
        "images": os.path.join(config.dataset_path, "valid"),
        "labels": os.path.join(config.dataset_path, "valid"),
    }
    ds = CachingImageProvider(
        im_type,
        paths_train,
        fn_mapping,
    ...
    if model is None and os.path.exists(
        os.path.join(config.weights_dir, "__checkpoint__.pth")
    ):
        print(
            "Will try to resume from the checkpoint: "
            + os.path.join(config.weights_dir, "__checkpoint__.pth")
        )
        ...
        checkpoint = None
    if model is None:
        if config.architecture == "DPNUNet":
            model = unet.DPNUnet(
                target_channel_num=config.target_channel_num,
                input_channel_num=config.input_channel_num,
            )
        elif config.architecture == "VQVAE":# Testing to train other models
            model = vqvae.VQVAE(
                in_channel=config.input_channel_num,
                out_channel=config.target_channel_num
            )
    ...
    print('----------Main model has %s parameters, %s of which is trainable----------' % (str(n_params), str(n_params_trainable)))
    ...
    estimator = Estimator(
        ...
        config.weights_dir,
        config=config,
        input_channel_num_changed=input_channel_num_changed,
        final_changed=False,
        device=config.device,
    )

    #estimator.lr_scheduler = ExponentialLR(estimator.optimizer, config.lr_gamma)  
    #estimator.lr_scheduler = LRStepScheduler(estimator.optimizer, config.lr_steps)
    estimator.lr_scheduler = ReduceLROnPlateau(estimator.optimizer, mode='min', factor=0.8, patience=3, verbose=True)
    callbacks = [
        ...
        # ModelFreezer(),
        PredictionSaver(
            config.outputs_dir, scale_target=True, report_interval=config.save_interval
        ),
        HistorySaver(config.logs_dir, config.outputs_dir, report_interval=config.save_interval),
        # EarlyStopper(10),
    ...
    if config.training_augmentations == "binarized":
        ...
        training_augmenter = aug_victor(0.97)
    elif config.training_augmentations == "minimal":
        training_augmenter = aug_minimal(0.9)
    else:
        raise NotImplementedError
    train_loader = PytorchDataLoader(
        TrainDataset(ds, train_idx, config, transforms=training_augmenter),
    ...
    val_loader = PytorchDataLoader(
        ValDataset(val_ds, val_idx, config, transforms=None),
        batch_size=1,
        shuffle=False,
        drop_last=False,
        pin_memory=True,
...
def start_segmentation_inference(config, apply_postprocessing=True):
    ...
    test_dataset = SequentialDataset(
        ds, test_idx, stage="test", config=config, transforms=None
    )
    test_dl = PytorchDataLoader(
        test_dataset,
    ...
    for data in pbar:
        ...
        input_shape = [
            round(float(input_shape[0]) * config.scale_factor),
            round(float(input_shape[1]) * config.scale_factor),
        ]
        ...
        for i in range(len(names)):
            ...
            outputs_file_name, outputs_file_extension = os.path.splitext(
                ds.fn_mapping["masks"](name)[0]
            )
            outputs_file = os.path.join(
                config.outputs_dir,
                outputs_file_name + "_output" + outputs_file_extension,
            )
            ...
            img = cv2.resize(
                img, (0, 0), fx=1 / config.scale_factor, fy=1 / config.scale_factor
            )
            ...
            if apply_postprocessing:
                ...
                image_label_overlay = label2rgb(
                    label_image, bg_label=0, bg_color=(0.8, 0.8, 0.8), colors=jet_colors
                )
                cv2.imwrite(
                    base_path + "_color_labels.png",
                    bytescale(image_label_overlay).astype("uint8"),
                )


def start_nuclei_seeded_segementation(
    config, nuclei_model, nuclei_channel_filename, cell_model
):
...
def start_regression_inference(config):
    ...
    for data in pbar:
        ...
        input_shape = data["image_shape"][:-1]
        input_shape = [
            int(float(input_shape[0]) * config.scale_factor),
            int(float(input_shape[1]) * config.scale_factor),
        ]
        predicted = predict8tta(model, samples, config.activation)
        names = data["image_name"]
        for i in range(len(names)):
...
def start_regression_inference_cellcycle(config):
    import cv2
    ...
    for data in pbar:
        ...
        for i in range(len(names)):
            name = names[i]
            name = os.path.split(name)[-1]
            _, outputs_file_extension = os.path.splitext(
                ds.fn_mapping["masks"](name)[0]
            )
            outputs_file_extension = '.png'
            if not os.path.exists(os.path.join(config.folder, "prediction", name)):
                os.makedirs(os.path.join(config.folder, "prediction", name))

            # saving paths
            outputs_file = os.path.join(
                config.folder, "prediction", name, "_output" + outputs_file_extension
            )
            inputs_file = os.path.join(
                config.folder, "prediction", name, "_inputs" + outputs_file_extension
            )
            targets_file = os.path.join(
                config.folder, "prediction", name, "_targets" + outputs_file_extension
            )
            ...
            prediction = cut_border(predicted[i, ...], input_shape)
            #print(name, 'Output', prediction.max())
            print(name, 'Output', prediction[:,:,0].max(), prediction[:,:,1].max(),prediction[:,:,2].max())
            img = (np.clip(prediction * 255, 0, 255)).astype(np.uint8)
            inputs = cut_border(samples[i].permute(1, 2, 0).numpy(), input_shape)
            inputs = inputs*normalize['std'] + normalize['mean']
            print(name, 'Input', inputs[:,:,0].max(), inputs[:,:,2].max())
            inputs = (np.clip(inputs * 255, 0, 255)).astype(np.uint8)
            target = cut_border(targets[i].permute(1, 2, 0).numpy(), input_shape)
            print(name, 'Target', target[:,:,0].max(), target[:,:,1].max(), target[:,:,2].max())
            target = (np.clip(target * 255, 0, 2552)).astype(np.uint8)
            if config.scale_factor != 1.0:
                img = cv2.resize(
                    img, (0, 0), fx=1 / config.scale_factor, fy=1 / config.scale_factor
                )
                inputs = cv2.resize(
                    inputs,
                    (0, 0),
                    fx=1 / config.scale_factor,
                    fy=1 / config.scale_factor,
                )
                target = cv2.resize(
                    target,
                    (0, 0),
            ...
            print(img.shape, img.max(), inputs.max(), target.max(), img.dtype)
            imwrite(outputs_file, img)
            imwrite(inputs_file, inputs)
            imwrite(targets_file, target)
