...
def merge_files(root):
    res_path = os.path.join(
        "..", "..", "predictions", os.path.split(root)[-1] + "_test"
    )
...
def cut_border(image, target_shape):
    ...
    return (
        image
        if not target_shape
        else image[32 : 32 + target_shape[0], 32 : 32 + target_shape[1], ...]
    )
...
def _gen_masks(datasets_dir, file_ids, border_detection_threshold=6):
    ...
    for i, file_id in enumerate(file_ids):
        ...
        try:
            gen_mask_from_geojson(
                [file_path],
                masks_to_create_value=["border_mask"],
...
def generate_geojson_masks(datasets_dir, border_detection_threshold=6):
    ...
    if os.path.exists(os.path.join(datasets_dir, "valid")):
        ...
        _gen_masks(
            os.path.join(datasets_dir, "valid"),
            file_ids,
            border_detection_threshold=border_detection_threshold,
        )
...
def allocate_gpu(num=1, gpu_ids=None):
    ...
    if gpu_ids is None:
        ...
        DEVICE_ID_LIST = GPUtil.getAvailable(
            order="first", limit=num, maxLoad=0.8, maxMemory=0.8
        )
...
def rescale_intensity(img):
    # p0, p99 = np.percentile(img, (0, 99))
    return exposure.rescale_intensity(img, in_range=(0, 125))
...
def get_preprocessing(type):
    if type == "identity":
        ...
        return lambda x: (exposure.equalize_adapthist(x, clip_limit=0.02)*255).astype("uint8")
    else:
        raise NotImplementedError
