...
def read_img_stack_v2(data_path):
    # Create a list to store all frames
    frames = []
    img = Image.open(data_path)
    # Loop through all frames/pages
    try:
        while True:
            # Append the current frame to our list
    ...
    # Shape will be (num_frames, height, width) for grayscale
    # or (num_frames, height, width, channels) for RGB
    img_stack = np.array(frames)

    nucleic_img_stack = img_stack[:21, :, :]
    protein_img_stack = img_stack[63:84, :, :]
    cilia_img_stack = img_stack[21:42, :, :]
    bb_img_stack = img_stack[84:, :, :]
...
def read_img_stack(data_path):
    ...
    for i in range(30):
        ...
        for i in directory.rglob(suffix):
            try:
                nucleic_img_stack.append(imread(i))
            except:
    ...
    for i in range(30):
        ...
        for i in directory.rglob(suffix):
            try:
                protein_img_stack.append(imread(i))
            except:
    ...
    for i in range(30):
        ...
        for i in directory.rglob(suffix):
            try:
                cilia_img_stack.append(imread(i))
            except:
